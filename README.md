...
| GET    | `/api/dashboard-top-summary` | Aggregate top-X previews in one fetch (4 top-X panels + compare_preview + cache stats). Default limit=5/panel, max 20. Optional `?material=` and `?city=` filters. Designed for dashboard overview — reduces N HTTP fetches to 1. (iter #62) |
