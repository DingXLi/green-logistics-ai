...
| GET    | `/api/persistence/vehicle-stats` | Per-vehicle historical aggregates: routes, km, cost, CO₂, efficiency (iter #41) |
| GET    | `/api/persistence/supply-aggregates` | Per-supply cumulative KPIs (iter #15) |
| GET    | `/api/persistence/demand-aggregates` | Per-demand cumulative KPIs + fulfillment rate (iter #52) |
| GET    | `/api/persistence/cycle-duration-stats` | Solver wall-time distribution (mean/p50/p95/slow_cycles) (iter #53) |
...
| GET    | `/api/persistence/compare-cycles` | Side-by-side comparison of two optimization cycles. Returns both cycles' full KPIs + derived metrics (cost_per_ton / co2_per_ton / cost_per_km / co2_per_km / avg_tons_per_match), absolute + pct differences, and 5-axis winner (lowest CO₂/ton, lowest cost/ton, highest util, most matches, most tons). (iter #59) |
| GET    | `/api/persistence/compare-materials` | Side-by-side comparison of two materials (volume / activity / efficiency / sustainability / demand context). Returns both materials' full KPIs (n_matches, match_rate, total_matched_tons, total_co2_kg, co2_per_ton, total_cost_sek, cost_per_ton, demand_fulfillment_pct, etc.) + absolute + pct differences + 5-axis winner (lowest CO₂/ton, lowest cost/ton, most matches, highest match_rate, highest demand_fulfillment). Required `material_a` + `material_b` params. Optional `since_sim_day` / `until_sim_day` window. Per-cycle route co2/cost allocated by match.tons proportion. (iter #64) |
| GET    | `/api/persistence/compare-routes` | Side-by-side comparison of two specific route rows (by primary key `route_id`). Returns both routes' full KPIs (distance_km, duration_hours, cost_sek, co2_kg, cycle_tons, n_stops, co2_per_km, co2_per_hour, cost_per_km, cost_per_hour, speed_km_per_hour) + absolute + pct differences + 5-axis winner (lowest CO₂/km, lowest CO₂/hour, lowest cost/km, highest speed, lowest duration). Required `route_id_a` + `route_id_b` params. (iter #64) |
| GET    | `/api/persistence/cycle-duration-histogram` | Cycle solver-duration histogram with 8 fixed buckets (<100ms / 100-500ms / 0.5-1s / 1-5s / 5-10s / 10-30s / 30-60s / 60s+). Each bucket reports count + pct. Includes summary stats (mean/median/min/max/stddev + slow/fast counts). Designed for histogram chart in dashboard. (iter #64) |
| GET    | `/api/persistence/cycle-duration-by-problem-size` | Cycle duration + efficiency breakdown by problem size. Buckets cycles by `total_tons` into small (<10t) / medium (10-50t) / large (50-200t) / xlarge (>=200t). Per-bucket stats: wall_duration_ms (mean/median/min/max/stddev), cost_sek, cost_per_ton_sek, distance_km, n_matches, solver_status breakdown (optimal/feasible/infeasible/unknown), dominant material_type. Returns `scaling_signal` (linear / superlinear / sublinear / unknown) — compares large vs small bucket duration ratio to tonnage ratio. Optional `since_sim_day` / `until_sim_day` filters. (iter #67) |
| GET    | `/api/persistence/solver-duration-trend` | Solver wall_duration_ms p50/p95 trend over chronological time windows. Each window contains N cycles (default 5, clamped [2, 30]). Per-window stats: p50 / p95 / mean / min / max / stddev. Overall trend (improving / declining / stable / unknown) compares first-half vs second-half median p50 with 10% threshold. Includes `trend_delta_pct` and `trend_confidence` (0-1). Optional `window_size` / `since_sim_day` / `until_sim_day` params. Designed for time-series trend chart in dashboard. (iter #68) |
| GET    | `/api/persistence/solver-duration-by-season` | Solver wall_duration_ms breakdown by season (winter/spring/summer/fall) based on `seasonal_month`. Winter=Dec–Feb, spring=Mar–May, summer=Jun–Aug, fall=Sep–Nov. Per-season stats: p50 / p95 / mean / min / max / stddev + solver_status counts (OPTIMAL/FEASIBLE/INFEASIBLE/UNKNOWN). Includes `global_stats` (across all seasons) + `slowest_season` + `fastest_season` + `slowest_vs_fastest_pct` (delta p50). Optional `since_sim_day` / `until_sim_day` filters. Designed for seasonal comparison chart in dashboard. (iter #69) |
| GET    | `/api/persistence/solver-duration-by-status` | Solver wall_duration_ms breakdown by solver_status (OPTIMAL/FEASIBLE/INFEASIBLE/UNKNOWN). Per-status stats: p50 / p95 / mean / min / max / stddev + `pct_of_total` + `mean_cost_sek` + `mean_cost_per_ton_sek` + `mean_n_matches` + `mean_tons`. Includes `global_stats` + `slowest_status` + `fastest_status` + `slowest_vs_fastest_pct` + SLO-style rate pills (`optimal_rate_pct` / `feasible_rate_pct` / `infeasible_rate_pct`). Optional `since_sim_day` / `until_sim_day` filters. Combined with iter #69 gives 3D view (time/season/status) of solver performance. (iter #71) |
| GET    | `/api/persistence/solver-duration-by-season-status` | Solver wall_duration_ms 3D heatmap (season × status). Returns a 4×4 = 16-cell matrix (winter/spring/summer/fall × OPTIMAL/FEASIBLE/INFEASIBLE/UNKNOWN). Each cell has n_cycles + p50/p95/mean/min/max/stddev + `pct_of_total` + `mean_cost_sek` + `mean_cost_per_ton_sek`. Includes `season_rollup` + `status_rollup` (1D marginals), `global_stats`, `slowest_cell` + `fastest_cell` + `slowest_vs_fastest_pct`, and `top_5_slowest_cells` (hotspot triage). Optional `since_sim_day` / `until_sim_day` filters. Designed for the 3D heatmap panel in dashboard. (iter #72) |
| GET    | `/api/persistence/vehicle-timeseries` | Per-vehicle time-series of efficiency metrics. Returns one row per cycle × per vehicle with the chosen metric (co2_per_km / co2_per_hour / cost_per_km / cost_per_hour / speed_km_per_hour / distance_km / n_stops). Includes `per_vehicle_summary` dict with mean/min/max/latest + trend classification (improving / declining / stable, 10% threshold). Optional `vehicle_id` + `since_sim_day`/`until_sim_day` filters. (iter #65) |
| GET    | `/api/persistence/cycle-trend-comparison` | Compare first N cycles vs last N cycles by chosen metric. Returns mean/min/max/median/stddev for each window + absolute + pct delta + trend classification (improving / declining / stable, 5% threshold). 7 metrics supported (co2_per_ton / cost_per_ton / co2_per_km / cost_per_km / fleet_utilization_pct / match_rate_vs_offers / tons_per_cycle). Default windows: 5 early + 5 late. (iter #65) |
| GET    | `/api/persistence/material-timeseries` | Per-material time-series of volume / efficiency metrics. Returns one row per cycle × per material with the chosen metric (matched_tons / n_matches / avg_match_tons / co2_per_ton / cost_per_ton / match_rate). Includes `per_material_summary` dict with mean/min/max/latest + trend classification (improving / declining / stable, 10% threshold). Optional `material_type` + `since_sim_day`/`until_sim_day` filters. Per-cycle route co2/cost allocated by match.tons proportion. (iter #65) |
| GET    | `/api/dashboard-top-summary` | Aggregate top-X previews in one fetch (5 top-X panels + compare_preview + cache stats). Default limit=5/panel, max 20. Optional `?material=` and `?city=` filters. Designed for dashboard overview — reduces N HTTP fetches to 1. (iter #62 + iter #63 top_routes) |
| GET    | `/api/persistence/top-routes` | Top routes (per cycle × per vehicle) ranked by efficiency (co2_per_km / co2_per_hour / cost_per_km / cost_per_hour / speed_km_per_hour / distance / tons_per_km) with optional `vehicle_id` and `since_sim_day`/`until_sim_day` filters. Surfaces which individual route rows are most efficient. (iter #63) |
| GET    | `/api/persistence/top-materials` | Top materials ranked by volume / efficiency (total_matched_tons / total_supply_tons / total_demand_tons / n_matches / n_supply_offers / n_demand_requests / match_rate / avg_tons_per_match / co2_per_ton / cost_per_ton / avg_distance_km) with optional `since_sim_day`/`until_sim_day` filters. Each row carries full context (n_matches, n_supply_offers, n_demand_requests, match_rate, demand_fulfillment_pct, avg_distance_km, total_co2_kg, total_cost_sek). Materials with 0 matches AND 0 offers are excluded. (iter #63) |
